    ...
    def test_routes(self, current_valve, test_route_data):
        ...
        for i in range(self.useful_valves_count):
            ...
            if(duration < self.maximum_duration):
                ...
                if test_route_data.best_pressure is None or pressure > test_route_data.best_pressure:
                    test_route_data.best_pressure = pressure
                    test_route_data.best_route = copy.deepcopy(test_route_data.route_segment_compound_compound)
...
def main():
    ...
    assert(test_route_data.best_route is not None)
    print(test_route_data.best_route)
    print(test_route_data.best_route.get_detail_route_description(volcano))
    test_route_data.best_route.calculate_pressure(debug=True)
